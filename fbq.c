...
process *temp_q1[MAX_PROCESSES+1];
process *temp_q2[MAX_PROCESSES+1];



int temp_s = 0;  // size of temp queue
int temp_s1 = 0;  // size of temp queue
int temp_s2 = 0;  // size of temp queue
...
void wait_temp(void){ 
	...
	for(i = 0; i < W_size; i++){
		...
		if((W->bursts[W->currentBurst].step) == (W->bursts[W->currentBurst].length)){ 
            ...
            // check if the current length is less than q1
            if((W->bursts[W->currentBurst].length) < q_time){
                temp_q1[temp_s1] = W;
            }
            else if (W->bursts[W->currentBurst].length < q_time2){
                temp_q2[temp_s2] = W;
            }
...
process *scheduler(void){
        ...
        else if (ready.size > 0){ // thy are still processes in the ready queue
                
            next = ready.front->data; // dequeue the front process
    		dequeueProcess(&ready);
    		return next; //return the next process
        } else if(q1.size > 0){ // process in q1 array
            
            next = q1.front->data; // dequeue the front process
    		dequeueProcess(&q1);
    		return next; //return the next process
            
        } else if (q2.size>0){ // process in q2
            
            next = q2.front->data; // dequeue the front process
    		dequeueProcess(&q2);
    		return next; //return the next process
        }
