...
/* If the number of processes taken in as input is greater than the alogorithm can handle or no process is detected */ 
void error_input(int n){
    if (n == 0 ) {
        printf("No processes detected");
        exit(EXIT_FAILURE);
    } 
    if (n > MAX_PROCESSES){
